...
class MusicProfile extends React.Component {
  ...
  _selectSong = (song) => {
    if (!this.state.selected || (this.state.selected && this.state.selected.publicId !== song.publicId)) {
  ...
  _changeTab = async (list) => {
    ...
    this.setState({ loading: false, currentTab: list, musicList, selected: null });
  };
  _filterQuadrant = async (quadrant) => {
    ...
    if (!loading) {
      ...
      this.setState({ loading: false, musicList, emotionList, quadrant, mood: null, selected: null });
    }
  };
  _filterMood = async (mood) => {
    ...
    if (!loading) {
      ...
      const { musicList } = await getSongsPromise(currentTab.value, filter);
      this.setState({ loading: false, musicList, mood: mood.value, selected: null });
    }
  };
  render() {
    const { t } = this.props;
    ...
    return <div className="homepage-div">
      ...
      <div className="songDiv">
        ...
        <div className="list-player">
          <Muziekweb song={this.state.selected} autoPlay showInfo />
        </div>
        <div className="recommendation-container">
          {loading? <Loading /> : <SongsList current={this.state.selected} onItemClick={this._selectSong} list={musicList} t={t} quadrant={quadrant} mood={mood} />}
        </div>
      </div>
    </div>;
  }
